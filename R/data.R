...
#'
#' @format A data frame with approximately 6 rows and 19 columns. Same
#'   column structure as [essence_raw], plus `HasBeenI` (see Details); see
#'   [essence_raw]'s documentation for the other column definitions.
#'
#' @details
...
#' }
#'
#' `HasBeenI = 1` for every row here, mirroring `HasBeenAdmitted`, as real
#' ESSENCE data typically has them move together for a genuine admission.
#' Every `has_been_*` column, including `has_been_i`, is guaranteed a real
#' `0`/`1` value with no `NA` in this dataset and in [essence_ed_raw]; the
#' two combined never produce an `NA` `has_been_*` column in
#' [link_encounters()]'s output.
#'
#' @source Entirely synthetic. Generated by `data-raw/generate_synthetic_data.R`.
