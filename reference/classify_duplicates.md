...
- `"visit_date_change"`:
  ...
  `C_BioSense_ID` is derived from `C_Visit_Date` and
  `C_Visit_Date_Time`, which are frequently populated from
  `Admit_Date_Time`. When a hospital treats `C_Visit_Date_Time` as a
  modifiable field and submits an update that crosses midnight, NSSP
  computes a new `C_BioSense_ID` for the same `Visit_ID` – producing two
  ...
  detection alerts that do not reflect genuine changes in incidence. The
...
- `"pid_change"`:
  ...
  The patient identifier (`C_Unique_Patient_ID`, which maps to MRN in
  Kentucky) was updated or corrected mid-visit by the facility. Detected
  by multiple distinct `C_Unique_Patient_ID` values within a group. Not
  all ESSENCE sites use MRN as `C_Unique_Patient_ID`.
