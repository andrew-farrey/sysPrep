...
The `Region` field uses the format `SITE_County`, where `SITE` is a site
abbreviation and `County` is the county name. Because some site names
contain multiple underscores, the prefix is all characters before the
**last** underscore. A visit is classified as out-of-state when its
