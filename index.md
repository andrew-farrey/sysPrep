...
questions tolerate the data quality issues below without materially
affecting conclusions. They are most valuable for **small-count,
high-impact case definitions**, where external validity and minimizing
false-positive clusters/anomalies matter most. Raw ESSENCE data pulls
can present four categories of data quality issues that bias case counts
or distort cluster/anomaly detection if left unaddressed:
...
inconsistently when they choose to apply them at all. `sysPrep`
...
The methods formalized in `sysPrep` were developed through applied
opioid overdose surveillance and cluster detection work using NSSP
ESSENCE data.
