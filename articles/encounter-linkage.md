...
two different `C_BioSense_ID` values sharing one `Hospital x Visit_ID`
(the linking key `essence_ed_raw`/ `essence_inp_raw` resolve to by
default, since both carry `Hospital`; see
[`?dedupe`](https://andrew-farrey.github.io/sysPrep/reference/dedupe.md)’s
“Facility identifier preference” section), correctly merged into a
...
#> 4 Lakeside Community Hospital V86489009 Direct Admit
```
