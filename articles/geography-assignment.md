...
ESSENCE encodes county-level geography in the format `SITE_County`,
where `SITE` is the site abbreviation (e.g., `"KY"`) and `County` is the
county name (e.g., `"Jefferson"`). Some site names contain additional
underscores (e.g., multi-word state abbreviations used in certain NSSP
configurations), so the site prefix is defined as all characters before
